    ...
    int findMinSum1(vector<int> strs, vector<int> stbs)
    {
        ...
        for (auto it = matrix.begin(); it!=matrix.end(); ++it)
        {
            if(!isInVector(strs, str_pos))
            {
                ...
                for (const int &i:*it)
                {
                    if(i < m && i != 0 && !isInVector(stbs, stb_pos)) m = i;
                    ++stb_pos;
                };
                sum += m;
            }
    ...
    int findMinSum2(vector<int> strs, vector<int> stbs)
    {
        ...
        for (auto it = matrix[0].begin(); it!=matrix[0].end(); ++it)
        {
            if(!isInVector(stbs, stb_pos))
            {
                ...
                {
                    if((*i)[stb_pos] < m && (*i)[stb_pos] != 0 && !isInVector(strs, str_pos)) m = (*i)[stb_pos];
                    ++str_pos;
                };
                sum += m;
            }
...
public:
    GraphWidget(QWidget* parent = nullptr) : QWidget(parent) {
        ...
        connect(showMatrix, &QPushButton::clicked, this, &GraphWidget::ShowM);
    }
    ...
    void ShowM()
    {
        ...
        for (int i = 0 ; i < int(graph.size()); i++)
        {
            for (int j = 0 ; j < int(graph.size()); j++)
            {
                ...
                {
                    str += QString::number(graph[i][j]) + " ";
                }
            }
            str += "\n";
        }
        QMessageBox::information(this, "Матрица", str);
    }
    ...
    void removeVertex() {
        ...
        if (ok && vertexIndex >= 0 && vertexIndex <= int(graph.size())) {
    ...
    void addEdge() {
        ...
        if (ok && vertexIndex1 >= 0 && vertexIndex1 <= int(graph.size())) {
            int vertexIndex2 = QInputDialog::getInt(this, tr("Добавить ребро"), tr("Введите индекс второй вершины:"), 0, 0, INT_MAX, 1, &ok);
            if (ok && vertexIndex2 >= 0 && vertexIndex2 <= int(graph.size())) {
                int weight = QInputDialog::getInt(this, tr("Добавить ребро"), tr("Введите вес ребра из 1 вершины во 2:"), 0, 0, INT_MAX, 1, &ok);
                if (ok) {
                    int weight2 = QInputDialog::getInt(this, tr("Добавить ребро"), tr("Введите вес ребра из 2 вершины в 1:"), weight, 0, INT_MAX, 1, &ok);
                    if (ok) {
                        graph[vertexIndex1][vertexIndex2] = weight;
                        graph[vertexIndex2][vertexIndex1] = weight2;
                        update();
                    }
                }
            }
    ...
    void paintEvent(QPaintEvent* event) override {
        ...
        for (int i = 0; i < int(graph.size()); ++i) {
            for (int j = 0; j < int(graph.size()); ++j) {
                if (graph[i][j] != 0) {
                    ...
                    double kx = dy / vlen * radius;
                    double ky = -dx / vlen * radius;
                    ...
                    painter.drawLine(QPointF(x2 - kx + dx2 / 2 + dy2, y2 - ky + dy2 / 2 - dx2), QPointF(x2 - kx, y2 - ky));
                    painter.drawLine(QPointF(x2 - kx - dx2 / 2 + dy2, y2 - ky - dy2 / 2 - dx2), QPointF(x2 - kx, y2 - ky));

                    painter.drawText(QRectF((x1 + x2) / 2 - kx - dx2 - tS / 2 , (y1 + y2) / 2 - ky - dy2 - tS / 2, tS, tS), Qt::AlignCenter, QString::number(graph[i][j]));
                }
            }
        }
        ...
        for (int i = 0; i < int(path.size()) - 1; i++)
        {
            ...
            minDistance += graph[path[i]][path[i + 1]];
        }
        ...
        for (int i = 0; i < int(path.size()); ++i) {
            pathText += QString::number(path[i]);
    ...
    QPushButton* showMatrix = new QPushButton("Показать матрицу");
private:
...
int main(int argc, char** argv) {
    ...
    layout->addWidget(graphWidget.showMatrix);
